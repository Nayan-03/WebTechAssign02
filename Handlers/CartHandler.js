...
exports.getProductsFromCart = async (req, res) => {
    try {
        ...
        if (carts.length === 0) {
            return res.status(404).json({ message: 'No Products found in your cart!!' });
        }
...
exports.getProductByCartId = async (req, res) => {
    try {
        ...
        if (!cart) {
            return res.status(404).json({ message: 'Required cart is not founded!!' });
        }
...
exports.addProductInCart = async (req, res) => {
    ...
    try {
        ...
        if(newCart) {
            res.status(201).json({ message: 'Product added in cart added successfully' });
        }
    } catch (err) {
...
exports.updateProductInCart = async (req, res) => {
    try {
        ...
        if (!updatedCart) {
            return res.status(404).json({ message: 'product in your cart is not founded, please check your product!!' });
        }
